...
void ShelfBox::Update(Player& player)
{
...
bool ShelfBox::CanInteract(const Player& player) const
{
...
void ShelfBox::DebugDrawInteractRange(
    const Player& player
) const
{
    VECTOR playerPos = player.GetPos();
...
void ShelfBox::CheckPlayerCollision(
    Player& player
)
{
...
void ShelfBox::DebugDrawCollision() const
{
    SetUseLighting(FALSE);
    SetWriteZBuffer3D(FALSE);
    SetUseZBuffer3D(FALSE);
...
void ShelfBox::OpenByEnemy(void)
{
    // 閉まっている、または閉まり中なら開ける
    if (state_ == DoorState::Closed ||
        state_ == DoorState::Closing)
...
bool ShelfBox::IsPassableForEnemy(void) const
{
    // 完全に開いたら敵は通れる
    return state_ == DoorState::Open;
}

bool ShelfBox::ResolveEnemyCollision(
    VECTOR& enemyPos,
    float enemyRadius,
    float bottomY,
    float topY
