...
void ShelfDoor::Init()
{
    ...
    animAttachIndex_ =
        MV1AttachAnim(trans_.modelId, 1);

    animAttachIndex2_ =
        MV1AttachAnim(trans_.modelId, 2);
...
void ShelfDoor::Update(Player& player)
{
...
void ShelfDoor::UpdateAnimation()
{
    switch (state_)
    {
...
bool ShelfDoor::CanInteract(const Player& player) const
{
...
bool ShelfDoor::DrawInteractUI(const Player& player) const
{
...
void ShelfDoor::DebugDrawInteractRange(
    const Player& player
) const
{
    VECTOR playerPos = player.GetPos();
...
void ShelfDoor::UpdateCollision()
{
    float rotY =
...
void ShelfDoor::CheckPlayerCollision(
    Player& player
)
{
...
void ShelfDoor::DebugDrawCollision() const
{
    SetUseLighting(FALSE);
    SetWriteZBuffer3D(FALSE);
    SetUseZBuffer3D(FALSE);
...
void ShelfDoor::OpenByEnemy(void)
{
    // 閉まっている、または閉まり中なら開ける
    if (state_ == DoorState::Closed ||
        state_ == DoorState::Closing)
...
bool ShelfDoor::IsPassableForEnemy(void) const
{
    // 完全に開いたら敵は通れる
    return state_ == DoorState::Open;
}

bool ShelfDoor::ResolveEnemyCollision(
    VECTOR& enemyPos,
    float enemyRadius,
    float bottomY,
    float topY
...
bool ShelfDoor::IsEnemyNearDoor(
